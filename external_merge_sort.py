...
class extsort:
    ...
    def subdivide(self):
        ...
        self.input_file = stream.BufferedInputStream(file_path, self.buffer_size)
        ...
        while not self.input_file.end_of_stream():
    ...
    def write_records(self, file_path, record_list):
        ...
        output_stream = stream.BufferedOutputStream(file_path, self.buffer_size)
        output_stream.create()
        for record in record_list:
            output_stream.write_line(record)
        output_stream.close()
    ...
    def merge(self, file_list, num_index):
        ...
        for i in range(0, len(file_list)):
            ref = stream.BufferedInputStream(file_list[i], self.buffer_size)
            ref.open()
        ...
        for i in range(0, len(file_object_list)):
            ...
            record = f_object.read_line()  # read a record from file
            record = record.decode('utf8', errors='ignore')

            if record:  # record not empty
                temp_pq_record = []

                col_value = record.split(',')[int(sort_column) - 1]
                if self.is_type_integer(self.file_name,
                                        self.colNum):  # if column is a number, put it as number so that correct sorting is applied
        ...
        output_file_path = "temp/part_sort_" + str(num_index) + ".csv"
        out_stream = stream.BufferedOutputStream(output_file_path, self.buffer_size)
        out_stream.create()
        ...
        while not record_priority_queue.empty():
            ...
            out_stream.write_line(data)  # put data into output stream

            # push new record from the same stream into queue, if end of file is not reached
            if not file_object_list[file_obj_index].end_of_stream():
                record = file_object_list[file_obj_index].read_line()
